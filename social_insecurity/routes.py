...
def index():
    ...
    if login_form.validate_on_submit() and login_form.submit.data:
        ...
        user = sqlite.query(
            get_user, {"username": login_form.username.data}, one=True)
        ...
        sqlite.query(
            insert_user,
            {
                ...
                "password": register_form.password.data,  # TODO: gen hash
            }
        )
...
def friends(username: str):
    ...
    if friends_form.validate_on_submit():
        ...
        get_friends = """
            SELECT f_id
            FROM Friends
            WHERE u_id = :friend_id;
            """
        friends = sqlite.query(get_friends, {"friend_id": user["id"]})
